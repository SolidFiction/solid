...
import 'package:solid/page/chat_bubble.dart';
...
class _ChatState extends State<Chat> {
  ...
  Future sendmessage() async {
    ...
    final response = await http
        .post(Uri.https('adeoropelumi.com', 'solid/newchat.php'), body: {
  ...
  Future allocate() async {
    ...
    //Response gotten for body i.e response.body then assigned to variable
    final response = await http
        .post(Uri.https('adeoropelumi.com', 'solid/assign.php'), body: {
      "email": widget.email,
    });
    ...
    if (response.statusCode == 200) {
      if (jsonDecode(response.body) == "true") {
        ...
        print("updated");
      }else if(jsonDecode(response.body) == "allocated"){
        print("allocated");
      }
    }
  }
  ...
  Future afterchattingMessage() async {
    final response = await http
        .post(Uri.https('adeoropelumi.com', 'solid/chatmsgs.php'), body: {
      "account": 'abellilo',
      "email": widget.email
    });
    var responsBody = json.decode(response.body);
    ...
    if (chatdata?.length == secondaddChat) {
      ...
      for(int? q = chatdata?.length; q! <secondaddChat!; q++){
        ...
        chatMessage.add(ChatMessage(message: modify(responsBody[q]['0'].toString()), type: users== "user" ?MessageType.Sender:MessageType.Receiver),);
      }
